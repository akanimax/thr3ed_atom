...
def train_triplane_mlp_vol_mod_with_posed_images(
    vol_mod: VolumetricModel,
    ...
    output_dir: Path,
    # optional arguments:)
    random_initializer: Callable[[Tensor], Tensor] = torch.nn.init.xavier_uniform_,
    test_dataset: Optional[PosedImagesDataset] = None,
    image_batch_cache_size: int = 8,
    ...
    optimizer = torch.optim.Adam(
        params=[
            {"params": vol_mod.thre3d_repr.parameters(), "lr": learning_rate},
            {"params": render_mlp.parameters(), "lr": learning_rate},
        ],
        betas=(0.9, 0.999),
    )
    ...
    #  Training Loop                                                                      |
    # -------------------------------------------------------------------------------------
    for train_iteration in range(1, num_iterations + 1):
        # ---------------------------------------------------------------------------------
        #  Main Operations Performed Per Iteration                                        |
        # ---------------------------------------------------------------------------------
        # sample a batch rays and pixels for a single iteration
        ...
        # images are of shape [B x C x H x W] and pixels are [B * H * W x C]
        pixels = (
            images.permute(0, 2, 3, 1).reshape(-1, images.shape[1]).to(vol_mod.device)
        )
        ...
        # render a small chunk of rays and compute a loss on it
        specular_rendered_batch = vol_mod.render_rays(
            rays_batch, parallel_points_chunk_size=32768
        )
        specular_rendered_pixels_batch = specular_rendered_batch.colour

        # compute loss and perform gradient update
        # Main, specular loss
        total_loss = mse_loss(specular_rendered_pixels_batch, pixels_batch)
        ...
        if (
            ...
            loss_info_string = (
                f"Global Iteration: {global_step} "
                f"Stage Iteration: {train_iteration} "
                f"specular_loss: {specular_loss_value.item(): .3f} "
                f"specular_psnr: {specular_psnr_value.item(): .3f} "
            )
            log.info(loss_info_string)
        ...
        # step the learning rate schedulers
        if train_iteration % lr_decay_iterations == 0:
            lr_scheduler.step()
            new_lrs = [param_group["lr"] for param_group in optimizer.param_groups]
            log_string = f"Adjusted learning rate | learning rates: {new_lrs} "
            log.info(log_string)

        # generate rendered feedback visualizations
        if (
            global_step % feedback_freq == 0
            or train_iteration == 1
            or train_iteration == num_iterations
